...
from AllPairsDijkstra import all_pairs_dijkstra
...
def metric_closure(G,Q, weight="weight"):
    M = nx.Graph()
    # Gnodes = set(G)
    # check for connected graph while processing first node
    all_paths_iter = all_pairs_dijkstra(G,Q, weight=weight)
    ...
    # Gnodes.remove(u)
    # for v in Q:
    #     M.add_edge(u, v, distance=distance[v])
    # first node done -- now process the rest
    for u,v, (distance, path) in all_paths_iter:
        M.add_edge(u, v, distance=distance, path=path)
        # print("all path iter")
    return M
...
def steiner_tree(G, terminal_nodes, weight="weight"):
    ...
    H = metric_closure(G,terminal_nodes, weight=weight)
