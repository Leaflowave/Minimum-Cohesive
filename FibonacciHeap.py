...
def fheappush(heap, item,key):
    """insert in Corment et al."""
    heap.mapping[key]=Node(item)
    # print(heap.mapping[key].key)
    # print("push")
    heap.insert(heap.mapping[key])
    heap.all_items.add(key)
...
def fheappop(heap):
    ...
    heap.all_items.remove(item.key[1])
    return item.key
...
class Fheap:
    ...
    def decrease_key(self, x, k):
        ...
        k must be no larger than the current key of x. If by decreasing x's key to
...
if __name__ == '__main__':
    ...
    fheappush(f, [-1,'b'],'b')
    # f.decrease_key('a',-2)
    target_value=f.mapping['a'].key
    f.decrease_key(f.mapping['a'],[target_value[0]-2,target_value[1]])
    print(f.extract_min().key)
